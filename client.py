...
def start_client():
    ...
    try:
        # ── خدمة الاسم: لا نتعامل مع IP/Port مباشرة، بل باسم الخدمة فقط ──
        host, port = resolve(SERVICE_NAME)
        client.connect((host, port))
        print(f"[CONNECTED] Connected to '{SERVICE_NAME}' ({host}:{port})")
    except ValueError as e:
        print(f"[ERROR] {e}")
        sys.exit(1)
    except ConnectionRefusedError:
        print(f"[ERROR] Could not reach service '{SERVICE_NAME}'.")
