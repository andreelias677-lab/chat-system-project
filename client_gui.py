...
from name_service import resolve
...
class ChatApp(ctk.CTk):
    ...
    def _build_login_screen(self):
        ...
        ctk.CTkLabel(self.login_frame, text="Service Name", anchor="w").pack(padx=40, fill="x")
        self.service_entry = ctk.CTkEntry(self.login_frame, width=320, placeholder_text="chat-server")
        self.service_entry.pack(padx=40, pady=(4, 12))
    ...
    def _do_connect(self):
        ...
        threading.Thread(target=self._connect_thread,
                         args=(service_name, username), daemon=True).start()

    def _connect_thread(self, service_name, username):
        try:
            # ── خدمة الاسم: نترجم اسم الخدمة إلى (host, port) ──────
